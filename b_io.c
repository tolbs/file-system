...
int b_open (char * filename, int flags)
{
	...
	mfs_DIR* inode = getInode(filename);
 	...
 	if( access(filename, F_OK) == 0){ // it exists
       	printf("open function: %s file does exist.\n", filename);
   	} else { // does not exists
       	printf("open function: %s does not yet exist.\n", filename);
       	return (-1);
   	}
	...
	fcbArray[fd].buf = malloc (bufSize+1);
...
int b_write (int fd, char * buffer, int count)
{
	...
	int freeSpace = bufSize - fcbArray[fd].index;
	...
	printf("Copying first segment to fcb->buf+%d: %d bytes\n", fcbArray[fd].index, copyLength);
	memcpy(fcbArray[fd].buf+fcbArray[fd].index, buffer, copyLength);
	fcbArray[fd].index += copyLength;
	fcbArray[fd].index %= bufSize;
	...
	if(secondCopyLength != 0)
	{
		...
		if (indexOfBlock == -1){
			...
			printf("\n\nFCB buff:\n\n%s", fcbArray[fd].buf);
			writeBufferToInode(fcb->inode, fcbArray[fd].buf, copyLength + secondCopyLength, indexOfBlock);
		}
		fcb->index = 0;
		printf("Copying second segment to fcb->buf+%d: %d bytes\n", fcbArray[fd].index, secondCopyLength);
		memcpy(fcbArray[fd].buf+fcbArray[fd].index, buffer+copyLength, secondCopyLength);
		fcbArray[fd].index += secondCopyLength;
		fcbArray[fd].index %= bufSize;
	}
...
int b_read (int fd, char * buffer, int count) {
  ...
  //int bytesRemaining =fcb->buflen-fcb->index;
	...
	int bytesRemaining = fcbArray[fd].buflen - fcbArray[fd].index;
  ...
  else
  {
    ...
    memcpy(buffer, fcbArray[fd].buf+fcbArray[fd].index, bytesRemaining);

    if(fcb->eof)
    {
	printf("EOF reached. Exiting.\n");
	fcbArray[fd].index += bytesRemaining;
    ...
    int newBufferSize = fcb->buflen = strlen(fcbArray[fd].buf);
		printf("%d bytes read.\n", newBufferSize);
    ...
    printf("Second: Copying to %ld from %ld for %ld bytes.\n", buffer+bytesRemaining, fcbArray[fd].buf+fcbArray[fd].index, secondSegmentCount);
    memcpy(buffer+bytesRemaining, fcbArray[fd].buf+fcbArray[fd].index, secondSegmentCount);

    fcbArray[fd].index+=secondSegmentCount;
...
void b_close (int fd)
{
	...
	if (fcb->mode == WRITE && fcbArray[fd].index > 0)
	{ // write last bytes
		...
		else
		{
			...
			writeBufferToInode(fcb->inode, fcbArray[fd].buf, fcbArray[fd].index, indexOfBlock);
		}
